...
5. **Specify accessibility** — WCAG compliance, VoiceOver order, Dynamic Type behavior. Follow `${CLAUDE_SKILL_DIR}/../gsp-accessibility/SKILL.md` methodology when annotating screen states; `gsp-project-critique` will run a `/gsp-accessibility --design` pass against this output downstream.
