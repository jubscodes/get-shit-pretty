...
- **EARS quality guardrails for `/gsp-project-brief`.** New `gsp-project-brief/references/ears-quality.md` reference catalogs hedge words (`distinct`, `easily`, `quickly`, `within … cycle`, bare `mobile`/`desktop`, `savings` without a formula, `visible focus` without dimensions) that pass EARS form but fail the contract — downstream phases each invent their own answer. Loaded by the brief skill before AC writing. Spec template adds a `## Numeric Inputs` section for declaring formulas / rounding / source data when any AC references a computed value. Spec template's Acceptance Criteria block now requires paired semantics ACs (`AC-N.Na`) for every UI-rendering behavior AC. Step 2 of the SDD pivot, follows up on PR #232's pipeline eval finding that AC-2.2 (savings formula) and AC-3.1/3.2 (table a11y) were invented three different ways across phases.
- **A11y semantics ACs are now a hard gate for `/gsp-project-brief`.** Reference §5 expanded with 6 pair patterns by element kind (static content, toggle/radio, button, dialog, image, live region) plus an explicit definition of what counts as a UI-rendering AC. Brief skill Step 2.3 promoted paired-AC guidance from suggestion to gate — if any UI-rendering AC lacks a paired `AC-N.Ma`, the spec is not done. Quality standards block enumerates the gate. Spec template's example block shows the `AC-1.1a` numbering. Step 3 of the SDD pivot — closes the eval finding that critique invented `<th scope>` and build invented ARIA for AC-3.1/3.2 with nothing in the spec to verify against.
- **`## Breakpoints` section is now a required spec block for responsive projects.** New `## Breakpoints` section in the spec template — declared numeric thresholds with names (`breakpoints.mobile`, `breakpoints.desktop`). ACs referencing responsive behavior must cite a declared name, not a bare `mobile`/`desktop` string. Reference §4 expanded with two declared examples (mobile+desktop, mobile+tablet+desktop) and a documented common-drift pattern: the design phase silently inventing a tablet tier when the spec declared only two. Step 4 of the SDD pivot — closes the eval finding that the pricing project's `responsive.md:18` introduced a tablet breakpoint the spec never authorized, and AC-5.1's "mobile breakpoint" was never anchored to a number. Design-side enforcement (gsp-project-design refusing to invent breakpoints not in spec) lands in PR 5 alongside the design+critique merge.
